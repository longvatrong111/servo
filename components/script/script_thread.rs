...
impl ScriptThread {
    ...
    fn process_pending_input_events(&self, pipeline_id: PipelineId, can_gc: CanGc) {
        ...
        for event in document.take_pending_input_events().into_iter() {
            ...
            self.notify_input_event_completed(pipeline_id, event.event);
        }
        ScriptThread::set_user_interacting(false);
    }

    fn notify_input_event_completed<T: WebDriverInputEvent>(
        &self,
        pipeline_id: PipelineId,
        event: T,
    ) {
        if let Some(id) = event.webdriver_id() {
            self.senders
                .pipeline_to_constellation_sender
                .send((
                    pipeline_id,
                    ...
                    MouseButtonAction::Up => {
                        ...
                        if pixel_dist < 10.0 * document.window().device_pixel_ratio().get() {
                            ...
                            document.note_pending_input_event(ConstellationInputEvent {
                                ...
                                event: event.event.clone().with_webdriver_id(None),
                            });
                            document.note_pending_input_event(ConstellationInputEvent {
                                ...
                                .with_webdriver_id(event.event.webdriver_id()),
                            });
                            return;
                        }
                    },
