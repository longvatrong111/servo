...
pub(crate) struct WebViewRenderer {
    ...
    pending_touch_and_mouse_input_events: RefCell<VecDeque<InputEvent>>,
...
impl WebViewRenderer {
    pub(crate) fn new(
        ...
        Self {
            ...
            animating: false,
            pending_touch_and_mouse_input_events: Default::default(),
    ...
    pub(crate) fn dispatch_input_event(&mut self, event: InputEvent) {
        ...
        if self.epoch_not_synchronized.get() {
            self.pending_touch_and_mouse_input_events
                .borrow_mut()
                .push_back(event);
            return;
        }
    ...
    pub(crate) fn dispatch_pending_touch_and_mouse_input_events(&self) {
        while let Some(event) = self
            .pending_touch_and_mouse_input_events
            .borrow_mut()
            .pop_front()
        {
            // Events that do not need to do hit testing are sent directly to the
